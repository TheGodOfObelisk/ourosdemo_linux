...
bool ScanProgressMeter::beginOrEndTask(const struct timeval *now, const char *additional_info, bool beginning) {
  ...
  if (beginning) {
    ...
    printf("Initiating %s at %02d:%02d", scantypestr, tm->tm_hour, tm->tm_min);
    ...
    printf("Completed %s at %02d:%02d, %.2fs elapsed", scantypestr, tm->tm_hour, tm->tm_min, TIMEVAL_MSEC_SUBTRACT(*now, begin) / 1000.0);
...
void adjust_timeouts2(const struct timeval *sent,
  ...
  if (delta < 0 && delta > -50000) {
    if (o.debugging > 2)
      //log_write(LOG_STDOUT, "Small negative delta (probably due to libpcap time / gettimeofday() discrepancy) - adjusting from %lius to %dus\n", delta, 10000);
  	  printf("Small negative delta (probably due to libpcap time / gettimeofday() discrepancy) - adjusting from %lius to %dus\n", delta, 10000);
    delta = 10000;
  }
  ...
  if (to->srtt == -1 && to->rttvar == -1) {
    ...
    if (rttdelta > 1500000 && rttdelta > 3 * to->srtt + 2 * to->rttvar) {
      if (o.debugging) {
        ...
        printf("Bogus rttdelta: %ld (srtt %d) ... ignoring\n", rttdelta, to->srtt);
      }
      return;
    }
    to->srtt += rttdelta >> 3;
  ...
  if (o.debugging > 3) {
    ...
    printf("delta %ld ==> srtt: %d rttvar: %d to: %d\n", delta, to->srtt, to->rttvar, to->timeout);
  }
...
bool ScanProgressMeter::printStats(double perc_done,
  ...
  if (ltime) {
    ...
    printf("%s Timing: About %.2f%% done; ETC: %02d:%02d (%.f:%02.f:%02.f remaining)\n",
        scantypestr, perc_done * 100, ltime->tm_hour, ltime->tm_min,
        floor(time_left_s / 60.0 / 60.0),
        floor(fmod(time_left_s / 60.0, 60.0)),
        floor(fmod(time_left_s, 60.0)));
    ...
    //     floor(fmod(time_left_s / 60.0, 60.0)),
    //     floor(fmod(time_left_s, 60.0)));
    printf("Timing error: localtime(%f) is NULL\n", (double) timet);
    printf("%s Timing: About %.2f%% done; ETC: Unknown (%.f:%02.f:%02.f remaining)\n",
        scantypestr, perc_done * 100,
        floor(time_left_s / 60.0 / 60.0),
        floor(fmod(time_left_s / 60.0, 60.0)),
        floor(fmod(time_left_s, 60.0)));
