...
void PortList::setPortState(u16 portno, u8 protocol, int state) {
  ...
  if ((state == PORT_OPEN && o.verbose) || (o.debugging > 1)) {
    ...
    printf("Discovered %s port %hu/%s%s\n",
              statenum2str(state), portno,
              proto2ascii_lowercase(protocol), idstr? idstr : "");
...
bool PortList::isTCPwrapped(u16 portno) const {
  ...
  if (port == NULL) {
    if (o.debugging > 1) {
      ...
      printf("PortList::isTCPwrapped(%d) requested but port not in list\n", portno);
    }
    return false;
  } else if (!o.servicescan) {
    if (o.debugging > 1) {
      // log_write(LOG_STDOUT, "PortList::isTCPwrapped(%d) requested but service scan was never asked to be done\n", portno);
      printf("PortList::isTCPwrapped(%d) requested but service scan was never asked to be done\n", portno);
    }
    return false;
    ...
    if (o.debugging > 1) {
      ...
      printf("PortList::isTCPwrapped(%d) requested but port has not been service scanned yet\n", portno);
    }
    return false;
    ...
    if (o.debugging > 1) {
      ...
      printf("PortList::isTCPwrapped(%d) requested but service has no name\n", portno);
    }
    return false;
  } else {
